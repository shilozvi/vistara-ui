/**
 * 🎯 Vistara UI - FlexRow Component
 * "Command your Design."
 * 
 * רכיב פריסה גמיש לשורות עם Flexbox מתקדם
 */
...
const FlexRow = forwardRef(({ 
  ...
  // Responsive behavior
  responsive = false,
  breakpoint = 'md', // 'sm', 'md', 'lg' - when to stack vertically
    ...
    return normalizeStyle({
      ...
      ...(responsive && {
        [`@media (max-width: ${getBreakpointValue()})`]: {
          flexDirection: direction.includes('reverse') ? 'column-reverse' : 'column',
  ...
  return (
    <div
      ref={ref}
      className={`vistara-flex-row vistara-flex-row--${variant} vistara-flex-row--${size} ${className || ''}`}
